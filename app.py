...
def profile():
    ...
    if edit_form.validate_on_submit():
        if User.authenticate(user.username, edit_form.password.data):
            user.username = edit_form.username.data
            user.email = edit_form.email.data
            user.image_url = edit_form.image_url.data
            user.bio = edit_form.image_url.data
...
def liked_warbles(user_id):
    user = User.query.get_or_404(user_id)  # Replace with your method of getting the user
    liked_warbles = user.likes  # Assuming you have a relationship for liked messages

    return render_template('liked_warbles.html', liked_warbles=liked_warbles, user=user)
